...
class LFUDAPolicyFixture : public ::testing::Test {
  protected:
	...
	if (exists > 0) { /* Remote copy */
	  ...
	  if (dir->set(env->dpp, block, false, y) < 0)
	    return -1;
...
TEST_F(LFUDAPolicyFixture, RemoteGetBlockYield)
{
  spawn::spawn(io, [this] (spawn::yield_context yield) {
    ...
    ASSERT_EQ(0, dir->set(env->dpp, &victim, false, optional_yield{io, yield}));
    ...
    ASSERT_EQ(0, dir->set(env->dpp, block, false, optional_yield{io, yield}));
