...
TEST_F(PolicyTest, Parse1) {
  ...
  ASSERT_NO_THROW(p = Policy(cct.get(), arbitrary_tenant, example1));
  ASSERT_TRUE(p);
...
TEST_F(PolicyTest, Eval1) {
  auto p  = Policy(cct.get(), arbitrary_tenant, example1);
  Environment e;
...
TEST_F(PolicyTest, Parse2) {
  ...
  ASSERT_NO_THROW(p = Policy(cct.get(), arbitrary_tenant, example2));
  ASSERT_TRUE(p);
...
TEST_F(PolicyTest, Eval2) {
  auto p  = Policy(cct.get(), arbitrary_tenant, example2);
  Environment e;
...
TEST_F(PolicyTest, Parse3) {
  ...
  ASSERT_NO_THROW(p = Policy(cct.get(), arbitrary_tenant, example3));
  ASSERT_TRUE(p);
...
TEST_F(PolicyTest, Eval3) {
  auto p  = Policy(cct.get(), arbitrary_tenant, example3);
  Environment em;
...
const string PolicyTest::example1 = " \
{ \
  \"Version\": \"2012-10-17\", \
  \"Statement\": { \
    \"Effect\": \"Allow\", \
...
const string PolicyTest::example2 = " \
{									\
  \"Version\": \"2012-10-17\",						\
...
const string PolicyTest::example3 = " \
{						\
  \"Version\": \"2012-10-17\",			\
  \"Statement\": [				\
    {						\
