...
int Credentials::generateCredentials(CephContext* cct,
                          ...
                          const boost::optional<std::unordered_map<string, string> > token_claims,
...
AssumeRoleWithWebIdentityResponse STSService::assumeRoleWithWebIdentity(AssumeRoleWithWebIdentityRequest& req)
{
  ...
  token_claims.emplace("iss", req.getIss());
  token_claims.emplace("aud", req.getAud());
  token_claims.emplace("sub", req.getSub());
