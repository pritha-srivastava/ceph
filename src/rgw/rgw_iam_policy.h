...
#ifndef CEPH_RGW_POLICY_S3V2_H
#define CEPH_RGW_POLICY_S3V2_H
...
struct ARN {
  ...
  ARN(Partition p, Service s, std::string g, std::string a, std::string r)
    : partition(p), service(s), region(std::move(g)), account(std::move(a)),
      resource(std::move(r)) {}
  ARN(const rgw_obj& o);
...
struct Condition {
  ...
  // says keys are to be added at run-time not compile
  // time.
  ...
  Condition(TokenID op, const char* s, std::size_t len) : op(op) {
    static constexpr char ifexistsstr[] = "IfExists";
    static constexpr std::size_t ifexistslen = sizeof(ifexistsstr);
    auto l = memmem(static_cast<const void*>(s), len,
		    static_cast<const void*>(ifexistsstr), ifexistslen);
    if (l) {
      ifexists = true;
...
struct Policy {
  ...
  Policy(CephContext* cct, const std::string& tenant, std::string text);
