...
int RedisLFUDAPolicy::init(CephContext* cct, const DoutPrefixProvider* dpp, asio::io_context& io_context, rgw::sal::Driver* _driver) {
  ...
  try {
    ...
    redis_exec(conn->conn, ec, req, resp, y);
...
int RedisLFUDAPolicy::age_sync(const DoutPrefixProvider* dpp, optional_yield y) {
  ...
  if (std::get<0>(resp).value().value().empty() || age > std::stoi(std::get<0>(resp).value().value())) { /* Set new maximum age */
    try { 
      ...
      redis_exec(conn->conn, ec, req, ret, y);
...
asio::awaitable<void> RedisLFUDAPolicy::redis_sync(const DoutPrefixProvider* dpp, optional_yield y) {
  ...
  for (;;) try {
    ...
    if (int ret = age_sync(dpp, y) < 0) {
      ldpp_dout(dpp, 0) << "RedisLFUDAPolicy::" << __func__ << "() ERROR: " << ret << dendl;
    }
...
int RedisLFUDAPolicy::eviction(const DoutPrefixProvider* dpp, uint64_t size, optional_yield y) {
  int ret = -1;
...
void RedisLFUDAPolicy::cleaning(const DoutPrefixProvider* dpp)
{
