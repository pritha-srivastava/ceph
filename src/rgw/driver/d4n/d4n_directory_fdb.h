...
class FDBBucketDirectory: public FDBDirectory, public BucketDirectory {
  ...
  private:
    ...
    int zadd(const DoutPrefixProvider* dpp, const std::string& bucket_id, double score, const std::string& member, std::optional<CacheObject> params, optional_yield y);
    int zrem(const DoutPrefixProvider* dpp, const std::string& bucket_id, const std::string& member, optional_yield y);
    int zrange(const DoutPrefixProvider* dpp, const std::string& bucket_id, const std::string& start, uint64_t count, std::vector<CacheObject>& objs_info, std::string& continuation_token, bool start_inclusive, optional_yield y);
    int zscan(const DoutPrefixProvider* dpp, const std::string& bucket_id, const std::string& start_token, const std::string& prefix, uint64_t count, bool marker_inclusive, std::vector<CacheObject>& objs_info, std::string& continuation_token, optional_yield y);
...
class FDBObjectDirectory: public FDBDirectory, public ObjectDirectory {
  ...
  private:
    template<SeqContainer Container>
    int set_values(const DoutPrefixProvider* dpp, CacheObjectVersion& obj_version_info, Container& fdbValues, optional_yield y);
    ...
    int zrevrange(const DoutPrefixProvider* dpp, const std::string& bucket_id, const std::string& obj_name, const std::string& marker_version, uint64_t count, std::vector<std::string>& members, optional_yield y);
