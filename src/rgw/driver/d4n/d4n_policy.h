...
class CachePolicy {
  ...
  public:
    ...
    virtual bool update_refcount_if_key_exists(const DoutPrefixProvider* dpp, std::string& key, uint8_t op, optional_yield y) = 0;
...
class LFUDAPolicy : public CachePolicy {
  ...
  public:
    ...
    virtual bool update_refcount_if_key_exists(const DoutPrefixProvider* dpp, std::string& key, uint8_t op, optional_yield y) override;
...
class LRUPolicy : public CachePolicy {
  ...
  public:
    ...
    virtual int exist_key(std::string key) override;
    virtual int eviction(const DoutPrefixProvider* dpp, uint64_t size, optional_yield y) override;
    virtual bool update_refcount_if_key_exists(const DoutPrefixProvider* dpp, std::string& key, uint8_t op, optional_yield y) override { return false; }
    virtual void update(const DoutPrefixProvider* dpp, const std::string& key, uint64_t offset, uint64_t len, const std::string& version, bool dirty, uint8_t op, optional_yield y, std::string& restore_val=empty) override;
    virtual void update_dirty_object(const DoutPrefixProvider* dpp, const std::string& key, const std::string& version, bool dirty, uint64_t size, 
			    double creationTime, const rgw_user user, std::string& etag, const std::string& bucket_name, const std::string& bucket_id,
    			    const rgw_obj_key& obj_key, uint8_t op, optional_yield y, std::string& restore_val=empty) override;
