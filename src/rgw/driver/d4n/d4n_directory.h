...
class Pipeline {
  public:
    Pipeline(std::shared_ptr<connection>& conn) : conn(conn) { pipeline_mode = true; }
    ...
    friend class ObjectDirectory;
    friend class BucketDirectory;
...
class BucketDirectory: public Directory {
  public:
    ...
    int zadd(const DoutPrefixProvider* dpp, const std::string& bucket_id, double score, const std::string& member, optional_yield y, Pipeline* pipeline=nullptr, bool multi=false);
...
class ObjectDirectory: public Directory {
  public:
    ...
    int zadd(const DoutPrefixProvider* dpp, CacheObj* object, double score, const std::string& member, optional_yield y, Pipeline* pipeline=nullptr, bool multi=false);
