...
int ObjectDirectory::set(const DoutPrefixProvider* dpp, CacheObj* object, bool nx, optional_yield y) 
{
  if (nx) {
    if (exist_key(dpp, object, y) == 1) {
      ldpp_dout(dpp, 20) << "ObjectDirectory::" << __func__ << "(): Key already exists." << dendl;
      return 0;
    }
  }
...
int BlockDirectory::set(const DoutPrefixProvider* dpp, CacheBlock* block, bool nx, optional_yield y) 
{
  if (nx) {
    if (exist_key(dpp, block, y) == 1) {
      ldpp_dout(dpp, 20) << "BlockDirectory::" << __func__ << "(): Key already exists." << dendl;
      return 0;
    }
  }
...
int BlockDirectory::get(const DoutPrefixProvider* dpp, CacheBlock* block, optional_yield y) 
{
  ...
  try {
    ...
    block->cacheObj.dirty = to_bool(std::get<0>(resp).value().value()[8]);
