...
std::string RedisObjectDirectory::build_index(CacheObj* object) 
{
...
std::string RedisBlockDirectory::build_index(CacheBlock* block) 
{
...
int RedisBlockDirectory::set_values(const DoutPrefixProvider* dpp, CacheBlock& block, Container& redisValues, optional_yield y)
{
  ...
  if ((ret = check_bool(std::to_string(block.deleteMarker))) != -EINVAL) {
    ...
    ldpp_dout(dpp, 0) << "RedisBlockDirectory::" << __func__ << "() ERROR: Invalid bool value for delete marker" << dendl;
    return -EINVAL;
  }
