...
int RemoteCache::send_and_complete_request(const DoutPrefixProvider* dpp, bufferlist& bl, std::string method, optional_yield& y)
{
  ...
  if (ret < 0) {
    ldpp_dout(dpp, 20) << "RemoteCacheDelete:: " << __func__ <<  " send_request failed with ret: " << ret << dendl;
    return ret;
  }

  ret = complete_request(dpp, y);
  if (ret < 0) {
    ldpp_dout(dpp, 20) << "RemoteCacheDelete:: " << __func__ <<  " complete_request failed with ret: " << ret << dendl;
    return ret;
  }
