                              ...
                              optional_yield y)
{
  ...
  if (write_to_cache) {
    ...
    creationTime = ceph::real_clock::to_time_t(state.mtime);
    ...
    d4n_dest_object->set_attrs_from_obj_state(dpp, y, state, baseAttrs);
    dest_object->set_obj_state(state);
  } else {
    RGWObjState* s;
    dest_object->get_obj_state(dpp, &s, y);
    baseAttrs = dest_object->get_attrs();
    d4n_dest_object->set_attrs_from_obj_state(dpp, y, *s, baseAttrs);
    auto ret = d4n_dest_object->calculate_version(dpp, y, *s, dest_version);
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ...
      if (dirty) {
        ...
        driver->get_policy_driver()->get_cache_policy()->updateObj(dpp, object_key, dest_version, true, this->get_obj_size(), creationTime, std::get<rgw_user>(dest_object->get_bucket()->get_owner()), *etag, dest_object->get_bucket()->get_name(), dest_object->get_key(), y);
      }
                                ...
                                rgw_obj* target_obj)
{
  ...
  if ((ret = get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ...
    this->set_attrs_from_obj_state(dpp, y, *state, attrs);
...
int D4NFilterObject::D4NFilterReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
  ...
  if ((ret = source->get_obj_attrs_from_cache(dpp, y)) == -ENOENT) {
    ...
    source->set_attrs_from_obj_state(dpp, y, *state, attrs);
                       ...
                       uint32_t flags)
{
  ...
  if (d4n_writecache == true) {
    ...
    if (real_clock::is_zero(set_mtime)) {
      ...
      state.mtime = set_mtime;
    }
    if (mtime) {
      *mtime = state.mtime;
    }
    state.obj = object->get_obj();
    state.size = accounted_size;
    ...
    RGWObjState* s;
    object->get_obj_state(dpp, &s, y);
    attrs = object->get_attrs();
    object->set_attrs_from_obj_state(dpp, y, *s, attrs);

    std::string version;
    ret = object->calculate_version(dpp, y, *s, version);
    if (ret < 0 || version.empty()) {
      ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__ << "(): version could not be calculated." << dendl;
    }
  }
				    ...
				    rgw::sal::Object* target_obj)
{
  ...
  std::string head_oid_in_cache;
  rgw::sal::Attrs attrs;
  d4n_target_obj->get_obj_state(dpp, &state, y);
  attrs = d4n_target_obj->get_attrs();
  d4n_target_obj->set_attrs_from_obj_state(dpp, y, *state, attrs);
  bufferlist bl_val;
  ...
  std::string version;
  ret = d4n_target_obj->calculate_version(dpp, y, *state, version);
  if (ret < 0 || version.empty()) {
    ldpp_dout(dpp, 10) << "D4NFilterObject::" << __func__ << "(): version could not be calculated." << dendl;
  }
