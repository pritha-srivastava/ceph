  ...
  if (!source->get_obj_attrs_from_cache(dpp, y)) {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        ...
        source->driver->get_policy_driver()->get_cache_policy()->update(dpp, head_oid_in_cache, 0, bl.length(), version, false, creationTime, this->source->get_bucket()->get_owner(), y);
...
int D4NFilterWriter::prepare(optional_yield y) 
{
  ...
  if (!object->have_instance()) {
    ...
    version = buf; //version for non-versioned objects, using gen_rand_alphanumeric_no_underscore for the time being
...
int D4NFilterWriter::process(bufferlist&& data, uint64_t offset)
{
	...
	if (bl.length() > 0) {          
 	    ...
 	    driver->get_policy_driver()->get_cache_policy()->update(dpp, oid_in_cache, ofs, bl.length(), version, dirty, creationTime,  obj->get_bucket()->get_owner(), y);
            if (!blockDir->exist_key(&block, y)) {
              if (blockDir->set(&block, y) < 0) //should we revert previous steps if this step fails?
  	        ldpp_dout(dpp, 10) << "D4NFilterObject::D4NFilterWriteOp::" << __func__ << "(): BlockDirectory set method failed." << dendl;
            } else {
                       ...
                       uint32_t flags)
{
  ...
  if (d4n_writecache == true) {
    ...
    if (ret == 0) {
      ...
      driver->get_policy_driver()->get_cache_policy()->update(dpp, head_oid_in_cache, 0, bl.length(), version, dirty, creationTime, obj->get_bucket()->get_owner(), y);
      ...
      driver->get_policy_driver()->get_cache_policy()->updateObj(dpp, prefix, version, dirty, accounted_size, creationTime, obj->get_bucket()->get_owner(), objEtag, y);
