  ...
  if (write_to_cache) {
    ...
    if (bl.length() > 0 && last_part) { // if bl = bl_rem has data and this is the last part, write it to cache
      ...
      if (!filter->get_policy_driver()->get_cache_policy()->exist_key(oid)) {
	    ...
	    filter->get_policy_driver()->get_cache_policy()->updateObj(dpp, prefix, version, dirty, source->get_obj_size(), creationTime, source->get_bucket()->get_owner(), objEtag, source->get_bucket()->get_name(), source->get_key(), *y);
                       ...
                       uint32_t flags)
{
  ...
  if (d4n_writecache == true) {
    ...
    if (ret == 0) {
      ...
      driver->get_policy_driver()->get_cache_policy()->updateObj(dpp, prefix, version, dirty, accounted_size, creationTime, obj->get_bucket()->get_owner(), objEtag, obj->get_bucket()->get_name(), obj->get_key(), y);
