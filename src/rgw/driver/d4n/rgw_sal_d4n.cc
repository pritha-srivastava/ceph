  ...
  if ((params.part_num && !source->is_multipart()) || !params.part_num) {
    ...
    do {
      ...
      if (policy->update_refcount_if_key_exists(dpp, oid_in_cache, rgw::d4n::RefCount::INCR, y) > 0) {
        ...
        // Read From Cache
        auto completed = cache_driver->get_async(dpp, y, aio.get(), cache_key, read_ofs, len_to_read, cost, id);
  ...
  if (write_to_cache) {
    ...
    if (bl.length() > 0 && last_part) { // if bl = bl_rem has data and this is the last part, write it to cache
      ...
      if (!policy->exist_key(oid)) {
        ...
        if (ret == 0) {
          ret = cache_driver->put(dpp, cache_key, bl, bl.length(), attrs, *y);
          if (ret == 0) {
            std::string objEtag = "";
            policy->update(dpp, oid, adjusted_start_ofs, bl.length(), version, dirty, rgw::d4n::RefCount::NOOP, source->get_bucket()->get_bucket_id(), source->get_name(), *y);
            blocks.emplace_back(block);
          } else {
      ...
      if (source->dest_object && source->dest_bucket) {
        ...
        if (ret == 0) {
          ret = cache_driver->put(dpp, dest_cache_key, bl, bl.length(), attrs, *y);
          if (ret == 0) {
            policy->update(dpp, dest_oid, adjusted_start_ofs, bl.length(), dest_version, dirty, rgw::d4n::RefCount::NOOP, source->dest_object->get_bucket()->get_bucket_id(), source->dest_object->get_name(), *y);
      ...
      if (bl_rem.length() == rgw_max_chunk_size) {
        std::string oid = prefix + CACHE_DELIM + std::to_string(adjusted_start_ofs) + CACHE_DELIM + std::to_string(bl_rem.length());
          ...
          if (ret == 0) {
            ...
            ret = cache_driver->put(dpp, cache_key, bl_rem, bl_rem.length(), attrs, *y);
            if (ret == 0) {
              policy->update(dpp, oid, adjusted_start_ofs, bl_rem.length(), version, dirty, rgw::d4n::RefCount::NOOP, source->get_bucket()->get_bucket_id(), source->get_name(), *y);
              blocks.emplace_back(block);
            } else {
        ...
        if (source->dest_object && source->dest_bucket) {
          ...
          if (ret == 0) {
            ...
            ret = cache_driver->put(dpp, dest_cache_key, bl_rem, bl_rem.length(), attrs, *y);
            if (ret == 0) {
              policy->update(dpp, dest_oid, adjusted_start_ofs, bl_rem.length(), dest_version, dirty, rgw::d4n::RefCount::NOOP, source->dest_object->get_bucket()->get_bucket_id(), source->dest_object->get_name(), *y);
              dest_blocks.emplace_back(dest_block);
            }
          }
        }
