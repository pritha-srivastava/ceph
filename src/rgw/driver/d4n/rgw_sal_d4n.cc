...
int D4NFilterObject::set_head_block_dir_entry(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Attrs& attrs, bool is_latest_version, bool dirty)
{
  ...
  if (is_latest_version) {
    ...
    auto set_head_blocks = [&](rgw::d4n::Pipeline* p) -> int {
      ...
      if (int r = blockDir->set(dpp, &block, y, p); r < 0) {
    ...
    if (dirty) {
      ...
      if (int r = objDir->add_version(dpp, this->get_bucket()->get_bucket_id(), objName, object_version, mtime, ver_params, y, p); r < 0) {
      ...
      if (int r = bucketDir->add_object(dpp, this->get_bucket()->get_bucket_id(), this->get_name(), bucket_params, y, p); r < 0) {
...
bool D4NFilterObject::check_head_exists_in_cache_get_oid(const DoutPrefixProvider* dpp, std::string& head_oid_in_cache, rgw::sal::Attrs& attrs, rgw::d4n::CacheBlock& blk, optional_yield y, bool acquire_lease)
{
  ...
  if (this->is_remote_cache_request() && !this->have_instance() && !this->get_object_version().empty()) {
    ...
    if (blockDir->get(dpp, &ver_block, y) == 0) {
      blk = ver_block;
  ...
  if (!remote_cache_request) {
      ...
      if (!objDirty) {
        ...
        auto delete_or_tombstone = [&](rgw::d4n::CacheBlock* blk, const std::string& version_to_check, const char* desc) -> int {
          if (objDirty) {
            ...
            blk->invalid = true;
            return blockDir->set(dpp, blk, y);
          } else {
            // Clean object: delete immediately
            return blockDir->del(dpp, blk, y);
          }
        };
          ...
          if (ret == 0) {
            ...
            if (latest_block.version == block.version) {
              ...
              if (obj_versions.size() == 2) {
                ...
                ret = bucketDir->add_object(dpp, source->get_bucket()->get_bucket_id(), source->get_name(), promoted_params, y);
                if (ret < 0) {
    ...
    if (!source->get_bucket()->versioned()) {
      ...
      if (objDirty) {
        ...
        if ((ret = blockDir->set(dpp, &ver_head, y)) < 0) {
        ...
        if ((ret = blockDir->del(dpp, &ver_head, y)) < 0 && ret != -ENOENT) {
