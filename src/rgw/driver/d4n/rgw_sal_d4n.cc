                              ...
                              optional_yield y)
{
  if (g_conf()->d4n_writecache_enabled) {
    ...
    if (ret == 0) {
      ...
      driver->get_policy_driver()->get_cache_policy()->updateObj(dpp, object_key, this->dest_version, true, state.accounted_size, creationTime, std::get<rgw_user>(dest_object->get_bucket()->get_owner()), *etag, dest_object->get_bucket()->get_name(), dest_object->get_key(), y);
