...
static string get_opaque_string_from_marker(const cls_queue_marker& marker) {
  bufferlist bl, bl64;
  encode(marker, bl);
  bl.encode_base64(bl64);
  string opaque_marker = bl64.to_str();
  CLS_LOG(5, "INFO: get_opaque_string_from_marker: marker is %s\n", opaque_marker.c_str());
  return opaque_marker;
}

static cls_queue_marker get_marker_from_string(const string& opaque_marker) {
  bufferlist bl, bl64;
  cls_queue_marker marker = {0, 0};
  bl64.append(opaque_marker);
  try {
    bl.decode_base64(bl64);
  } catch (buffer::error& err) {
    CLS_LOG(5, "ERROR: get_marker_from_string: failed to decode base64 marker \n");
    return marker;
  }
  ...
  try {
    ...
    CLS_LOG(5, "ERROR: get_marker_from_string: failed to decode marker \n");
    return marker;
  }
  return marker;
}
...
int write_queue_head(cls_method_context_t hctx, cls_queue_head& head)
{
  ...
  bufferlist bl_encoded_len;
  encode(encoded_len, bl_encoded_len);

  bl.claim_append(bl_encoded_len);
...
int get_queue_head(cls_method_context_t hctx, cls_queue_head& head)
{
  ...
  if (encoded_len > (chunk_size - sizeof(uint64_t))) {
    chunk_size = (encoded_len - (chunk_size - sizeof(uint64_t)));
  ...
  it = bl_head.cbegin();
  it.seek(sizeof(uint16_t) + sizeof(uint64_t));
  try {
    decode(head, it);
  } catch (buffer::error& err) {
...
int init_queue(cls_method_context_t hctx, const cls_init_queue_op& op)
{
  ...
  if (op.has_urgent_data) {
    ...
    head.max_urgent_data_size = head.max_head_size - (sizeof(head.max_head_size) +
                              sizeof(head.max_urgent_data_size) + sizeof(head.queue_size) +
                              sizeof(head.tail) + sizeof(head.front));
  } else {
  ...
  CLS_LOG(10, "INFO: cls_init_queue_op queue actual size %lu", head.queue_size);
  CLS_LOG(10, "INFO: cls_init_queue_op head size %lu", head.max_head_size);
  CLS_LOG(10, "INFO: cls_init_queue_op queue front offset %lu", head.front.offset);
  CLS_LOG(10, "INFO: cls_init_queue_op queue max urgent data size %lu", head.max_urgent_data_size);

  write_queue_head(hctx, head);
  return 0;
}
...
int get_queue_size(cls_method_context_t hctx, cls_queue_get_size_ret& op_ret)
{
  ...
  CLS_LOG(10, "INFO: get_queue_size: size of queue is %lu\n", op_ret.queue_size);

  return 0;
}

int enqueue(cls_method_context_t hctx, const cls_enqueue_op& op, cls_queue_head& head)
{
  ...
  for (auto bl_data : op.bl_data_vec) {
    ...
    if (head.tail.offset >= head.front.offset) {
      ...
      if ((head.tail.offset + bl.length()) <= head.queue_size) {
        CLS_LOG(1, "INFO: enqueue: Writing data size and data: offset: %lu, size: %u\n", head.tail.offset, bl.length());
        ...
        if (bl.length() <= free_space_available) {
          ...
          CLS_LOG(1, "INFO: enqueue: Writing spliced data at offset: %lu and data size: %u\n", head.tail.offset, bl_data_before_wrap.length());
          ...
          CLS_LOG(1, "INFO: enqueue: Writing remaining data at offset: %lu and data size: %u\n", head.tail.offset, bl.length());
    ...
    CLS_LOG(1, "INFO: enqueue: New tail offset: %lu \n", head.tail.offset);
  } //end - for
...
int queue_list_entries(cls_method_context_t hctx, const cls_queue_list_op& op, cls_queue_list_ret& op_ret, cls_queue_head& head)
{
  ...
  if ((head.front.offset == head.tail.offset) && (head.front.gen == head.tail.gen)) {
    CLS_LOG(1, "INFO: queue_list_entries(): Next offset is %lu\n", head.front.offset);
    op_ret.next_marker = get_opaque_string_from_marker(head.front);
    op_ret.is_truncated = false;
    return 0;
  }

  cls_queue_marker start_marker = get_marker_from_string(op.start_marker);
  ...
  CLS_LOG(1, "INFO: queue_list_entries(): front is: %lu, tail is %lu\n", head.front.offset, head.tail.offset);
  ...
  do
  {
    CLS_LOG(1, "INFO: queue_list_entries(): start_offset is %lu\n", start_offset);
    ...
    CLS_LOG(1, "INFO: queue_list_entries(): size_to_read is %lu\n", size_to_read);
    if (size_to_read == 0) {
      next_marker = head.tail;
      op_ret.is_truncated = false;
      CLS_LOG(1, "INFO: queue_list_entries(): size_to_read is 0, hence breaking out!\n");
      break;
    }
    ...
    CLS_LOG(1, "INFO: queue_list_entries(): size of chunk %u\n", bl_chunk.length());
    ...
    do {
      CLS_LOG(1, "INFO: queue_list_entries(): index: %u, size_to_process: %lu\n", index, size_to_process);
      it.seek(index);
      //Populate offset if not done in previous iteration
      if (! offset_populated) {
        cls_queue_marker marker = {start_offset + index, gen};
        CLS_LOG(1, "INFO: queue_list_entries(): offset: %lu\n", marker.offset);
        string opaque_marker = get_opaque_string_from_marker(marker);
      ...
      if (! entry_start_processed ) {
        if (size_to_process >= (sizeof(uint16_t) + sizeof(uint64_t))) {
          ...
          try {
            ...
            CLS_LOG(1, "ERROR: queue_list_entries: failed to decode entry start \n");
            return -EINVAL;
          }
          if (entry_start != QUEUE_ENTRY_START) {
            CLS_LOG(1, "ERROR: queue_list_entries: invalid entry start %u\n", entry_start);
            return -EINVAL;
          }
          ...
          try {
            ...
            CLS_LOG(1, "ERROR: queue_list_entries: failed to decode data size \n");
            return -EINVAL;
          }
        } else {
          ...
          CLS_LOG(1, "INFO: queue_list_entries: not enough data to read entry start and data size, breaking out!\n");
          break;
        }
        CLS_LOG(1, "INFO: queue_list_entries(): data size: %lu\n", data_size);
      ...
      if (data_size <= size_to_process) {
        ...
        CLS_LOG(1, "INFO: queue_list_entries(): not enough data to read data, breaking out!\n");
        break;
      }
      ...
      num_ops++;
      if (num_ops == op.max) {
        CLS_LOG(1, "INFO: queue_list_entries(): num_ops is same as op.max, hence breaking out from inner loop!\n");
        break;
      }
    ...
    if (num_ops == op.max) {
      ...
      CLS_LOG(1, "INFO: queue_list_entries(): num_ops is same as op.max, hence breaking out from outer loop with next offset: %lu\n", next_marker.offset);
      break;
    }
    ...
    if (contiguous_data_size == 0) {
      if (wrap_around) {
        ...
        CLS_LOG(1, "INFO: queue_list_entries(): end of queue data is reached, hence breaking out from outer loop!\n");
        next_marker = head.tail;
        op_ret.is_truncated = false;
        break;
      }
  ...
  CLS_LOG(1, "INFO: queue_list_entries(): next offset: %lu\n", next_marker.offset);
  op_ret.next_marker = get_opaque_string_from_marker(next_marker);
...
int queue_remove_entries(cls_method_context_t hctx, const cls_queue_remove_op& op, cls_queue_head& head)
{
  ...
  cls_queue_marker end_marker = get_marker_from_string(op.end_marker);

  CLS_LOG(1, "INFO: queue_remove_entries: op.end_marker = %lu\n", end_marker.offset);
  ...
  if (end_marker.offset > head.front.offset && end_marker.gen == head.front.gen) {
    ...
    if (len > 0) {
      ...
      if (ret < 0) {
        CLS_LOG(1, "INFO: queue_remove_entries: Failed to zero out entries\n");
        CLS_LOG(1, "INFO: queue_remove_entries: Start offset = %lu\n", head.front.offset);
        CLS_LOG(1, "INFO: queue_remove_entries: Length = %lu\n", len);
        return ret;
      }
    }
    ...
    CLS_LOG(1, "INFO: queue_remove_entries: Invalid end marker: offset = %lu, gen = %lu\n", end_marker.offset, end_marker.gen);
    return -EINVAL;
  }
  ...
  CLS_LOG(1, "INFO: queue_remove_entries: front offset is: %lu and tail offset is %lu\n", head.front.offset, head.tail.offset);
